...
-(void)initLayout:(UIView<ActionOperator>*)_associateView
{
    ...
    //控制按钮
    CGPoint ptCenterControlBtn;
    ptCenterControlBtn.x = self.associateView.frame.origin.x+self.associateView.frame.size.width;
    ptCenterControlBtn.y = self.associateView.frame.origin.y+self.associateView.frame.size.height;
    CGRect rcControlBtn = CGRectMake(ptCenterControlBtn.x-EDIT_BTN_WIDTH/2,
                                   ptCenterControlBtn.y-EDIT_BTN_HIGHT/2,
                                   EDIT_BTN_WIDTH,
                                   EDIT_BTN_HIGHT);
    m_controlBtn = [[SSControlBtn alloc]initWithFrame:rcControlBtn];
    m_controlBtn.controlView = self.associateView;
    [m_controlBtn setBackgroundColor:[UIColor blueColor]];
    [m_controlBtn setTitle:@"S" forState:UIControlStateNormal];
    [self addControlGestureToView:m_controlBtn];
    assert([self.associateView.superview  isKindOfClass:[SSStageView class]]);
    [self.associateView.superview addSubview:m_controlBtn];
...
- (void)addControlGestureToView:(UIView *)view
{
    SSOneFingerControlGestureRecognizer *control = [[SSOneFingerControlGestureRecognizer alloc] initWithTarget:self action:@selector(handleControl:)];
    [view addGestureRecognizer:control];
}

//处理控制
- (void)handleControl:(SSOneFingerControlGestureRecognizer *)recognizer
{
    UIView *view = ((SSControlBtn *)recognizer.view).controlView;
    ...
    CGPoint ptMoveOffset = recognizer.ptMoveOffset;
    CGFloat rotateAngle = recognizer.rotateAngle;
    CGPoint ptScale = recognizer.ptScale;
    
    [view setTransform:CGAffineTransformMakeTranslation(ptMoveOffset.x, ptMoveOffset.y)];
    [view setTransform:CGAffineTransformRotate(view.transform, rotateAngle)];
    [view setTransform:CGAffineTransformScale(view.transform, ptScale.x, ptScale.y)];
    
    [self caculateBtnCPointAfterControl:ptMoveOffset withAngle:rotateAngle withScale:ptScale];
    /*
    CGFloat factor = [recognizer scale];
    if (factor>-0.001&&factor<0.001)
        return ;
    // CGAffineTransform tranform = view.transform;
    NSLog(@"handleScale START view transform scale x=%f, y=%f", factor, factor);
    ...
    if( ((cptScale.x-cptDelete.x<(2*EDIT_BTN_WIDTH))&&(cptScale.y-cptDelete.y<(2*EDIT_BTN_HIGHT))) && (factor<0.0001) )
    {
        //距离很近，不能再缩放了。
        return ;
    }
    [view setTransform:CGAffineTransformScale(view.transform, 1+factor, 1+factor)];
    [self caculateBtnCPointAfterScale:1+factor withYScale:1+factor];
     //*/
}
...
-(void)locateControlBtnPositionByBtnCenterPoint:(CGPoint)cptControl
{
    [m_controlBtn setFrame:[self getControlBtnPos:cptControl]];
}
...
-(void)caculateBtnCPointAfterControl:(CGPoint)ptMoveOffset
                           withAngle:(CGFloat)rotateAngle
                        withScale:(CGPoint)ptScale
{
//    if ((fxScale>-0.001&&fxScale<0.001) && (fyScale>-0.001&&fyScale<0.001))
//        return ;
    ...
    CGPoint cptControl;
    //缩放
    pt.x =  self.associateView.ptControlBtnOffsetPoint.x+ptMoveOffset.x;
    pt.y = self.associateView.ptControlBtnOffsetPoint.y+ptMoveOffset.y;
    pt.x = self.associateView.ptControlBtnOffsetPoint.x*ptScale.x;
    pt.y = self.associateView.ptControlBtnOffsetPoint.y*ptScale.y;
    self.associateView.ptControlBtnOffsetPoint = pt;
    self.associateView.ptControlBtnOffsetPoint = [self getPointReleateRotatePointCoordAfterRotate:self.associateView.ptMoveBtnOffestRotatePoint
                                                                                     withRotateAngle:rotateAngle];
    [self locateControlBtnPositionByBtnCenterPoint:cptControl];
}
